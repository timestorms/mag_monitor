    ...
    function __construct($options='') {
        ...
        if(empty($options)) {
            $options = array (
                ...
                'servers'   =>C('DATA_CACHE_SERVERS'),//,MEMCACHE服役器池
                'length'   =>0,
            );
        }
         ...
         * 
         */
        //加入了MEMCACHE分布式布署的支持  add by David Sun 2014-01-21
         $k=0;                          
        foreach ($options['servers'] as $servers) {
               $server_status=$this->handler->addServer($servers['ip'], $servers['port']);
               if(!$server_status) $k++; 
        }
        
        if($k==count($options['servers'])) $this->connected=FALSE;
        else $this->connected=TRUE;
  //打印出连接的服务器状态信息      
//        $stats = $this->handler->getExtendedStats();
//        var_dump($stats);
