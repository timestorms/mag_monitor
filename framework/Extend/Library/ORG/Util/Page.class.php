...
class Page {
    ...
    protected $config = array(
        ...
        'first' => '&laquo;',
		'last' => '&raquo;',
		'theme' => '<span class="info">%totalRow% %header% %nowPage%/%totalPage% 页</span> %upPage% %first% %linkPage% %downPage% %end%'
	);
    /**
     +----------------------------------------------------------
     * 架构函数
	...
	public function __construct($totalRows, $listRows, $rewrite = '0' ,$parameter = '')
    {
        ...
        if(!isset($_POST['FormPage'])){
        	...
        	$this->nowPage = !empty($_POST['FormPage']) ? $_POST['FormPage'] : 1;
        }
        if (!empty($this->totalPages) && $this->nowPage > $this->totalPages) {
            $this->nowPage = $this->totalPages;
        }
    ...
    public function show1(){
       ...
       if($startPage == 1){
           ...
           if($startPage >  2){
               ...
               $theFirst = "<a href='".$url.sprintf($format, 1)."' class='first' >". 1/*$this->config['first']*/ ."</a>";
           }
       ...
       if($endPage == $this->totalPages){
           ...
           $theEndRow = $this->totalPages;
           $nextPage = "[<a href='".$url.sprintf($format, $nextRow)."' >下".$this->rollPage."页</a>]";
           if(($this->totalPages - $this->nowPage) > 3){
               $downPage = "<a class='dotsm'>...</a> <a href='".$url.sprintf($format, $theEndRow)."' class='last' >". $theEndRow /*$this->config['last']*/ ."</a>";
           }  else {
               $downPage = "<a href='".$url.sprintf($format, $theEndRow)."' class='last'>". $theEndRow /*$this->config['last']*/ ."</a>";
           }
       ...
       $theEnd .="<form id='pageform' action='".$url."' method='post'>";
       $theEnd .= "<select name='FormPage'>";
       for($i=1;$i<=$this->totalPages;$i++){
               $selected = '';
               if($i==$this->nowPage){
                       $selected = 'selected';
               }
               $theEnd .= "<option value='".$i."' ".$selected.">".$i."</option>";
       }
       $theEnd .= "</select>";
       $theEnd .= "<input type='submit' value='转至' >";
       $theEnd .= "</form>";
