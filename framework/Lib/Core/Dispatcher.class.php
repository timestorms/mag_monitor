...
class Dispatcher {
    ...
    static public function dispatch() {
        ...
        if(!empty($_SERVER['PATH_INFO'])) {
            ...
            if(!self::routerCheck()){   // 检测路由规则 如果没有则按默认规则调度URL
                ...
                $var  =  array();
				//模块别名支持
				$group_alias = C('APP_GROUP_ALIAS');
				if ($group_alias && array_key_exists($paths[0], $group_alias)) {
					$paths[0] = $group_alias[$paths[0]];
				}
    ...
    static private function getGroup($var) {
		...
		$group_alias = C('APP_GROUP_ALIAS');
		if ($group_alias && isset($_GET[C('VAR_GROUP')])) {
			// 记录当前别名
            define('GROUP_ALIAS',strtolower($group));
			$group = $group_alias[$group];
		}
        unset($_GET[$var]);
