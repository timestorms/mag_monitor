...
class DbMysql extends Db{
    ...
    public function errorLog($str){
        $error_log_file = RUNTIME_PATH . 'tp3.0_sql_.' . date('Y-m-d') . '.log';
        error_log(date('Y-m-d H:i:s') . ":------------------------" . PHP_EOL, '3', $error_log_file);
        error_log($_SERVER['HTTP_HOST'] . ':' . MODULE_NAME . '/' . ACTION_NAME . PHP_EOL, '3', $error_log_file);
        error_log('SQL:' . $str . PHP_EOL, '3', $error_log_file);
        error_log('GET:' . http_build_query($_GET) . PHP_EOL, '3', $error_log_file);
        error_log('POST:' . http_build_query($_POST) . PHP_EOL, '3', $error_log_file);
    }
    ...
    public function query($str) {
        if (stripos($str, 'union') !== false && stripos($str, '_union') === false && stripos($str, 'union_') === false) {
            $this->errorLog($str);
            return false;
        }
        ...
        // 记录开始执行时间
        G('queryStartTime');
        log_message('Query SQL:'.$str,LOG_DEBUG);
        $this->queryID = mysql_query($str, $this->_linkID);
        $this->debug();
        if ( false === $this->queryID ) {
            $this->error();
            if (function_exists('log_message')) {
                log_message('DB query error:'.$this->error, LOG_EMERG);                
            }
            return false;
        } else {
    ...
    public function execute($str) {
        ...
        // 记录开始执行时间
        G('queryStartTime');
        log_message('Exec SQL:'.$str,LOG_DEBUG);
        $result =   mysql_query($str, $this->_linkID) ;
        $this->debug();
        if ( false === $result) {
            $this->error();
            if (function_exists('log_message')) {
                log_message('DB exec error:'.$this->error, LOG_EMERG);
            }
            return false;
        } else {
    ...
    private function getAll() {
        ...
        $result = array();
        if($this->numRows >0) {
			//*** Gui Gui <sd2536888@163.com> start ***
			$field = $this->fieldData();
			//*** Gui Gui <sd2536888@163.com> end ***
	...
	public function fieldData() {
		$retval = array();
		for ($i = 0, $c = mysql_num_fields($this->queryID); $i < $c; $i++)
		{
			$field = mysql_fetch_field($this->queryID, $i);
			$retval[$field->name]                = new stdClass();
			$retval[$field->name]->name        = $field->name;
			$retval[$field->name]->type        = $field->type;
			$retval[$field->name]->max_length  = $field->max_length;
		}
    ...
    public function getFields($tableName) {
        $tempArr = explode(' ',$tableName); //去掉查询表别名导致的空格问题
        if(!empty($tempArr)){
            $tableName = $tempArr[0];
        }
    ...
    public function error() {
        ...
        if($this->debug && '' != $this->queryStr){
            $this->error .= " [ SQL语句 ] : ".$this->queryStr;
        }
        return $this->error;
    }
